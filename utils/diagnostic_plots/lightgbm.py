"""LightGBM model diagnostic plots."""
...
def generate_lightgbm_plots(model=None, X_train=None, y_train=None, X_test=None, y_test=None,
                         feature_names=None, class_names=None, is_classifier=True,
                         learning_rates=None, tree_depths=None, sample_weights=None):
    """Generate diagnostic plots for LightGBM models.
    ...
    try:
        ...
        try:
            ...
            plots.append({
                ...
                "interpretation": "Shows the relative importance of each feature in the model. Features with higher importance scores have more influence on the model's predictions. LightGBM's feature importance represents how much each feature contributes to reducing the loss function across all trees."
            })
    ...
    # Plot 6: Split Value Histogram
    try:
        if hasattr(model, 'booster_') and hasattr(model.booster_, 'trees_to_dataframe'):
            plt.figure(figsize=(12, 6))
            
            # Get tree structure as DataFrame
            trees_df = model.booster_.trees_to_dataframe()
            
            # Extract split values (non-leaf nodes)
            split_values = trees_df[trees_df['split_gain'] > 0]['threshold'].values
            
            if len(split_values) > 0:
                sns.histplot(split_values, kde=True)
                plt.xlabel('Split Values')
                plt.ylabel('Frequency')
                plt.title('Distribution of Split Values Across Trees')
                plt.grid(True, alpha=0.3)
                
                plots.append({
                    "title": "Split Value Distribution",
                    "img_data": get_base64_plot(),
                    "interpretation": "Shows the distribution of threshold values used for splits across all trees. This can help identify key decision boundaries in the feature space and understand how LightGBM is partitioning the data."
                })
    except Exception as e:
        # This is optional, so we'll just pass if it fails
        pass
    
    # Plot 7: Tree Structure Visualization (if possible)
    try:
        if hasattr(model, 'booster_') and hasattr(model.booster_, 'plot_tree'):
            # Just plot the first tree (index 0)
            plt.figure(figsize=(15, 10))
            
            model.booster_.plot_tree(tree_index=0, figsize=(15, 10))
            plt.title('LightGBM Tree Structure (First Tree)')
            
            plots.append({
                "title": "Tree Structure",
                "img_data": get_base64_plot(),
                "interpretation": "Visualizes the structure of the first decision tree in the LightGBM ensemble. Each node represents a splitting rule, with leaf nodes showing the predicted values. This provides insight into how the model makes decisions but represents only one tree from many in the ensemble."
            })
    except Exception as e:
        # This is optional, so we'll just pass if it fails
        pass
